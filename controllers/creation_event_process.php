...
if (!empty($event_name_register) &&
    ...
    !empty($event_price_info_register) &&
    !empty($event_nbplace_register) && 
    !empty($is_no_limit_register) && 
    !empty($is_free_entry_register)) {
    createEvent($db, $event_name_register, $event_date_register, $event_time_register, $event_adress_register, $event_description_register, $event_category_register, $event_price_register, $event_price_info_register, $event_nbplace_register, $is_no_limit_register, $is_free_entry_register);
    $message="Votre évènement a bien été créé";
    $type_message = "success";
    header('Location: event_page#123456.php?message=' . $message . '&type_message=' . $type_message);
}
